...
class Object3D:
    def __init__(self, ctx, obj_path, camera, position=glm.vec3(0,0,0)):
        self.ctx = ctx
...
class Ring:
    ...
    def __init__(self, ctx, camera, radius=0.9, thickness=0.15, segments=128, color=(1.0, 0.2, 0.2), position=glm.vec3(0,0,0)):
        self.ctx = ctx
        self.camera = camera
        self.radius = radius
...
class ViewerApp:
    def __init__(self, person_with_ring_path, person_no_ring_path, win_size=(600, 600)):
        pg.init()
        ...
        # Models
        self.person_with_ring = Object3D(self.ctx, person_with_ring_path, self.camera, glm.vec3(0,0,0))
        self.person_no_ring = Object3D(self.ctx, person_no_ring_path, self.camera, glm.vec3(2,0,0))
        self.ring = Ring(self.ctx, self.camera, radius=0.9, thickness=0.15, position=self.person_with_ring.position + glm.vec3(0,0.05,0))

        self.ring_transferred = False
        self.clock = pg.time.Clock()

    def check_ring_touch(self):
        if self.ring_transferred:
            return
        ring_pos = self.ring.position
        person_pos = self.person_no_ring.position
        dist = glm.length(ring_pos - person_pos)
        if dist < 0.5:  # llindar de contacte
            self.transfer_ring()

    def transfer_ring(self):
        # Mou l'anell a la segona persona
        self.ring.position = self.person_no_ring.position + glm.vec3(0,0.05,0)
        self.ring_transferred = True
        print("Ring transferred to second person!")
            ...
            for e in pg.event.get():
                if e.type == pg.QUIT or (e.type == pg.KEYDOWN and e.key == pg.K_ESCAPE):
                    ...
                    self.ring = Ring(
                        ...
                        position=self.ring.position
                    )

            # Moviment de la segona persona amb WASD
            keys = pg.key.get_pressed()
            speed = 0.05
            if keys[pg.K_w]:
                self.person_no_ring.position.z -= speed
            if keys[pg.K_s]:
                self.person_no_ring.position.z += speed
            if keys[pg.K_a]:
                self.person_no_ring.position.x -= speed
            if keys[pg.K_d]:
                self.person_no_ring.position.x += speed

            # Comprova si toca l'anell
            self.check_ring_touch()

            # Render
            self.ctx.clear(0.07, 0.07, 0.09)
            self.person_with_ring.render()
            self.person_no_ring.render()
            self.ring.render()
...
if __name__ == "__main__":
    person_with_ring_path = "person2.obj"
    person_no_ring_path = "person2_noring.obj"
    app = ViewerApp(person_with_ring_path, person_no_ring_path)
    app.run()
